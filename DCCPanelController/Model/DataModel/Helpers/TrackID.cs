...
public static class TrackID {
    public static string NextTurnoutID(IEnumerable<TrackTurnout> tracks) => Next(tracks, "T");
    public static string NextButtonID(IEnumerable<TrackButton> tracks) => Next(tracks, "B");
    public static string Next(IEnumerable<ITrackID> tracks, string prefix)=>GetNextID(tracks, t => t.Id, prefix);
    public static string GetNextID<ITrackID>(IEnumerable<ITrackID> tracks, Func<ITrackID, string> idSelector, string defaultPrefix = "UKN") {
        var ids = tracks.Select(idSelector).OrderBy(id => id).ToList();
