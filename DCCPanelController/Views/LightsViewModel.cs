    ...
    private async Task SortByColumnAsync(string columnName) {
        ...
        if (!_isAscending) {
            sorted = columnName switch {
                _labelName  => Enumerable.OrderBy<Light, string>(Lights, x => x.Name ?? "").ToList(),
                _labelID    => Enumerable.OrderBy<Light, string>(Lights, x => x.Id ?? "").ToList(),
                _labelState => Enumerable.OrderBy<Light, bool>(Lights, x => x.State).ToList(),
                _           => Enumerable.ToList<Light>(Lights)
            };
        } else {
            sorted = columnName switch {
                _labelName  => Enumerable.OrderByDescending<Light, string>(Lights, x => x.Name ?? "").ToList(),
                _labelID    => Enumerable.OrderByDescending<Light, string>(Lights, x => x.Id ?? "").ToList(),
                _labelState => Enumerable.OrderByDescending<Light, bool>(Lights, x => x.State).ToList(),
                _           => Enumerable.ToList<Light>(Lights)
            };
        }
