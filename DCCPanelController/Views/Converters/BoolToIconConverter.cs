...
public class MultiBoolToIconConverter : IMultiValueConverter {
    ...
    public string ActiveIcon { get; set; } = "default_active.png";
    public string InactiveIcon { get; set; } = "default_inactive.png";
    
    public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture) {
        
        if (values is not { Length: 4 }) return "default.png";
        ...
        var firstName = values[2]?.ToString() ?? "default";

        // Parameter 4: string (second base name option)
        var secondName = values[3]?.ToString() ?? "default";
