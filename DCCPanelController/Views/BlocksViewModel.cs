    ...
    private async Task SortByColumnAsync(string columnName) {
        ...
        if (!_isAscending) {
            sortedBlocks = columnName switch {
                _labelName  => Enumerable.OrderBy<Block, string>(Blocks, x => x.Name ?? "").ToList(),
                _labelID    => Enumerable.OrderBy<Block, string>(Blocks, x => x.Id ?? "").ToList(),
                _labelState => Enumerable.OrderBy<Block, bool>(Blocks, x => x.IsOccupied).ToList(),
                _           => Enumerable.ToList<Block>(Blocks)
            };
        } else {
            sortedBlocks = columnName switch {
                _labelName  => Enumerable.OrderByDescending<Block, string>(Blocks, x => x.Name ?? "").ToList(),
                _labelID    => Enumerable.OrderByDescending<Block, string>(Blocks, x => x.Id ?? "").ToList(),
                _labelState => Enumerable.OrderByDescending<Block, bool>(Blocks, x => x.IsOccupied).ToList(),
                _           => Enumerable.ToList<Block>(Blocks)
            };
        }
