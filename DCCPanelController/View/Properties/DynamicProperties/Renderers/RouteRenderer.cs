...
internal sealed class RouteRenderer : IPropertyRenderer {
    public bool CanRender(PropertyContext ctx) => ctx.EditorKind == EditorKinds.Route;

    public object CreateView(PropertyContext ctx) {
        var row = ctx.Row;
        var label = new Label { Text = "Route", VerticalTextAlignment = TextAlignment.Center, HorizontalTextAlignment = TextAlignment.Start, Opacity = 0.6 };
