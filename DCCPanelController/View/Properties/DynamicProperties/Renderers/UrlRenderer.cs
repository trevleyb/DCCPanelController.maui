...
internal sealed class UrlRenderer : BaseRenderer, IPropertyRenderer {
    ...
    public object CreateView(PropertyContext ctx) {
        try {
        var row = ctx.Row;
        var entry = new Entry { Keyboard = Keyboard.Url, Text = row.OriginalValue?.ToString(), Placeholder = row.HasMixedValues ? "— mixed —" : "https://", TextColor = Colors.Black, };
        ...
        return WrapWithLabel(ctx, entry);
            ...
            Logger.LogError(ex, "Error creating Url Renderer for property {PropertyName}", ctx.Row?.Field?.Meta?.Label);
