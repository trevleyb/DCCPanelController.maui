...
internal sealed class TurnoutRenderer : IPropertyRenderer {
    public bool CanRender(PropertyContext ctx) => ctx.EditorKind == EditorKinds.Turnout;

    public object CreateView(PropertyContext ctx) {
        var row = ctx.Row;
        var label = new Label { Text = "Turnout", VerticalTextAlignment = TextAlignment.Center, HorizontalTextAlignment = TextAlignment.Start, Opacity = 0.6 };
