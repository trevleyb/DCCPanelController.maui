...
internal sealed class BlockRenderer : IPropertyRenderer {
    public bool CanRender(PropertyContext ctx) => ctx.EditorKind == EditorKinds.Block;

    public object CreateView(PropertyContext ctx) {
        var row = ctx.Row;
        var label = new Label { Text = "Block", VerticalTextAlignment = TextAlignment.Center, HorizontalTextAlignment = TextAlignment.Start, Opacity = 0.6 };
