...
namespace DCCPanelController.View.Properties {
    public partial class PropertyPopup : Popup {
        ...
        private const double ScreenMarginForPopup = 15; // Total margin (e.g., 20px on each of the 4 sides from screen edge)
        private const double MinPopupWidth = 280;       // Minimum sensible width for the popup
        private const double MinPopupHeight = 200;      // Minimum sensible height for the popup

        public PropertyPopup(IPropertiesViewModel viewModel) {
            InitializeComponent();
            ViewModel = viewModel;
            BindingContext = this; // To bind to ViewModel.Title in XAML
        ...
        private void UpdatePopupSizeConstraints()
        {
            ...
            double maxAllowedWidth = screenWidthInDips - ScreenMarginForPopup;
            double maxAllowedHeight = screenHeightInDips - ScreenMarginForPopup;
        ...
        public new async Task<object?> ShowAsync(Page? page = null) {
