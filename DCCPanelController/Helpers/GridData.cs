...
// --------------------------------------------------------------------------------------------------
public class GridData(bool ok, int? xOffset, int? yOffset, int? xCenter, int? yCenter, int? boxSize) {
    ...
    public GridData(int? xOffset, int? yOffset, int? xCenter, int? yCenter, int? boxSize) 
        : this(true, xOffset, yOffset, xCenter, yCenter, boxSize) { }

    // Helper if the data is invalid. Up to the caller to check the result
    // -------------------------------------------------------------------
    public static GridData Error(string? error = "") =>
        new GridData(false, null, null, null, null, null) { ErrorMessage = error ?? "" };
}
