...
namespace DCCClients.Test.Discovery {
    ...
    public class RealServiceDiscoveryTests {
        ...
        public async Task DiscoverJmriServers_ShouldFindRealJmriServers() {
            ...
            var servers = await jmriDiscovery.DiscoverServersAsync(DiscoveryTimeoutSeconds);
            ...
            var urls = await jmriDiscovery.DiscoverServerUrlsAsync(1); // Short timeout since we already discovered
            TestContext.Progress.WriteLine($"\nJMRI Server URLs: {urls}");
        ...
        public async Task DiscoverWiThrottleServers_ShouldFindRealWiThrottleServers() {
            ...
            var servers = await wiThrottleDiscovery.DiscoverServersAsync(DiscoveryTimeoutSeconds);
            var elapsedMs = Stopwatch.GetElapsedTime(startTime).TotalMilliseconds;
            TestContext.Progress.WriteLine($"Discovery completed in {elapsedMs:F2}ms");
            ...
            var addresses = await wiThrottleDiscovery.DiscoverServerUrlsAsync(1); // Short timeout since we already discovered
        ...
        public async Task DiscoveryPerformance_ShouldCompleteQuickly() {
            ...
            var jmriServers = await jmriDiscovery.DiscoverServersAsync(5);
            ...
            var wiThrottleServers = await wiThrottleDiscovery.DiscoverServersAsync(5);
